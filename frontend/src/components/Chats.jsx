...
const Chats = () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const filteredChatrooms = data.filter(chatroom => 
              !chatroom.authorizedUsers.some(user => user.id === userId)
            );
            setChatrooms(filteredChatrooms); // Filtrer les chatrooms pour cet utilisateur
          } else {
    ...
    }, [userId]); // Utiliser userId dans le tableau de dépendances pour rafraîchir les chatrooms lorsque l'utilisateur change
          ...
          setChatrooms((prevChatrooms) =>
            prevChatrooms.map((chatroom) =>
              chatroom.id === chatroomId
                ? {
                    ...chatroom,
                    askingUsers: [...chatroom.askingUsers, { id: userId }],
                  }
                : chatroom
            )
          );
        } catch (error) {
          console.error("Erreur lors de la demande de connexion à la chatroom :", error);
        }
      };
    ...
    return (
      <ProfilContainer>
        ...
        <Sidebar>
          ...
          <SidebarButton>Other Chats</SidebarButton>
        </Sidebar>
          ...
          <ChatroomContainer>
            {chatrooms.slice(currentIndex, currentIndex + 3).map((chatroom, index) => {
              ...
              return (
                ...
                <JoinIcon src="/img/join.png" onClick={() => handleJoinChatroom(chatroom.id)}/>
